...
namespace D2TxtImporter.lib.Model.Dictionaries
{
    public class MagicSuffix
    {
        ...
        // ClassSpecific is a string code that should resolve to a class name using CharStats
        ...
        public static void Import(string excelFolder)
        {
            ...
            foreach (var row in table)
            {
                ...
                {
                    NameKey = row.ContainsKey("name") ? row["name"] : row.ContainsKey("Name") ? row["Name"] : $"suffix_{index}",
                    Index = index - 1,
                    Level = level,
                ...
                if (propInfos.Count > 0)
                {
                    try
                    {
                        ...
                        magicSuffix.Properties = properties;
                    }
                    catch (Exception ex)
                    {
                        var affix = row.ContainsKey("name") ? row["name"] : (row.ContainsKey("Name") ? row["Name"] : "<unknown>");
                        throw new Exception($"Failed to build properties for MagicSuffix '{affix}'", ex);
                    }
                }
